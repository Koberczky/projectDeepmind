...
function virtualNav(data) {
    if (window.sessionStorage.getItem("imageNum") != null && restart == 0) {
        sessvars.data = data;
...
function clearTour(data) {
    ...
    sessionStorage.removeItem("imageNum");
    virtualNav(sessvars.data);
    return;
}

function RestartTour(data) {
    buildingNum = sessionStorage.getItem("buildingNumber");
    imageNum = sessionStorage.getItem("imageNum");
    restart = 1;
    virtualNav(sessvars.data);
    return;
}
...
function prevImage(imageNum) {
    var backBuilding = 0;
    oldBuildingNum = buildingNum - 1;
    imageNum = imageNum - 1;
    for (k = buildPosArray[oldBuildingNum][0]; k <= buildPosArray[oldBuildingNum][1]; k++) {
        if (imageNum == buildPosArray[oldBuildingNum][1]) {
            backBuilding = 1;
        }
    }
    if (backBuilding == 1) {
...
function getImage(imageNum) {
    for (k = buildPosArray[buildingNum][0]; k <= buildPosArray[buildingNum][1]; k++) {
        if (imageNum == buildPosArray[buildingNum][1]) {
            isBuilding = 1;
        }
    }
    if (imageNum != null) {
        ...
        if (imageNum != 0) {
            document.getElementById('buildingInfo').innerHTML = "<p class='text-center'><a class='prevLink' href = 'javascript: void(0)' onclick = 'prevImage(" + imageNum + ")'>Previous Image</a><a class='nextLink' href = 'javascript: void(0)' onclick = 'nextImage(" + imageNum + ")'>Next Image</a></p><p>" + instructions[imageNum] + "</p>";
        } else {
            document.getElementById('buildingInfo').innerHTML = "<p class='text-center'><a class='nextLink' href = 'javascript: void(0)' onclick = 'nextImage(" + imageNum + ")'>Next Image</a></p><p>" + instructions[imageNum] + "</p>";
        }
    }
    if (isBuilding == 1) {
        document.getElementById('buildingInfo').innerHTML = " <p> Do you want to enter " + buildingInfo[buildingNum].fullTitle + "?<br /><a href = 'javascript:void(0)' onclick = 'waitBuilding(" + imageNum + ")'> Yes </a> <a href='javascript:void(0)' onclick='leaveBuilding(" + imageNum + ")'>No</a> </p>";
        isBuilding = 0;
    }

}

function waitBuilding(imageNum) {
    if (buildingNum == 14) {
    ...
    document.getElementById('buildingInfo').innerHTML = " <p id = 'enterBuilding'> <a href = 'javascript:void(0)' onclick = 'leaveBuilding(" + imageNum + ")'> Click when you're ready to go to " + buildingInfo[nextBuildingNum].fullTitle + "</a></p>";
}

function leaveBuilding(imageNum) {
    if (buildingNum == 14) {
    ...
    window.sessionStorage.setItem("imageNum", imageNum);
    getImage(imageNum);
